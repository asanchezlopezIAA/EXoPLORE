...
class UniformPrior:
    ...
    def log_prior(self, value: float) -> float:
        """Evaluate the log-uniform prior at ``value``.

        Returns ``-ln(high - low)`` (the log of the constant probability
        density ``1 / (high - low)``) when ``value`` is within [low, high],
        and ``-inf`` outside.  Using the normalisation constant rather than
        0 ensures that log-posterior values from different prior widths are
        ...
        -------
        float
            ``-ln(high - low)`` if ``low <= value <= high``, else ``-inf``.
        """
...
class GaussianPrior:
    ...
    def log_prior(self, value: float) -> float:
        """Evaluate the log-Gaussian prior at ``value``.

        Returns the log of the normal probability density
        ``-0.5 z² - ln(σ √(2π))`` where ``z = (value - mean) / sigma``.
        Useful as an informative prior when an independent constraint
        (e.g., RV-derived Kp or literature equilibrium temperature) should
        be incorporated into the retrieval posterior.
        ...
        -------
        float
            Log-probability density (always finite for any real ``value``).
        """
...
class LogUniformPrior:
    ...
    def log_prior(self, value: float) -> float:
        """Evaluate the log-uniform (Jeffreys) prior at ``value``.

        The log-uniform distribution has probability density
        ``1 / (value × ln(10) × (log_high - log_low))`` in linear space,
        i.e., equal probability per decade.  This is the standard prior
        for scale parameters such as VMRs or mixing ratios where the
        interesting range spans several orders of magnitude and there is
        no strong reason to prefer any particular decade.

        Parameters
        ----------
        value : float
            Physical parameter value (must be > 0).

        Returns
        -------
        float
            Log-probability density, or ``-inf`` if ``value <= 0`` or
            outside the prior support ``[10^log_low, 10^log_high]``.
        """
        if value <= 0:
            return -math.inf
...
class PriorSet:
    ...
    def n_params(self) -> int:
        """Number of free parameters in this prior set.

        The integer returned here must be passed to ``pymultinest.run``
        as the ``n_params`` argument and to ``pymultinest.Analyzer`` as
        ``n_params``.  The ordering of parameters in the unit-cube vector
        ``cube[0..n_params-1]`` must match exactly the insertion order in
        :attr:`param_names`, i.e., the order in which :meth:`add` was
        called.
        """
    ...
    def param_names(self) -> List[str]:
        ...
        The physical meaning of each entry depends on the prior set
        constructed by the caller.  For a standard 1-D retrieval built
        with :func:`standard_1d_prior_set` the order is:

        ``["log10_X", "K_p", "T_equ", "v_wind"]``

        where:

        * ``log10_X``, log₁₀ of the volume mixing ratio of the target
          species (dimensionless, typically in the range -8 to -1).
        * ``K_p``, planet RV semi-amplitude in km/s.
        * ``T_equ``, equilibrium temperature in K.
        * ``v_wind``, line-of-sight wind velocity offset in km/s.

        When ``include_beta=True``, a fifth entry ``"beta"`` is appended
        (the noise-scaling nuisance parameter, dimensionless).

        For custom prior sets the list reflects whatever names were passed
        to :meth:`add`, in insertion order.
        """
    ...
    def log_prior(self, params: np.ndarray) -> float:
        """Evaluate the total log-prior probability for a parameter vector.

        Computes the sum of log-prior values for each parameter by calling
        the ``log_prior`` method on the corresponding :class:`UniformPrior`,
        :class:`GaussianPrior`, or :class:`LogUniformPrior` instance.  This
        form is expected by MCMC samplers such as emcee, where the posterior
        is ``log_prior + log_likelihood``.

        For uniform priors the contribution is ``-ln(high - low)`` (constant
        inside bounds) or ``-inf`` outside bounds.  For Gaussian priors it is
        the standard ``-0.5 z²`` log-normal.

        Parameters
        ----------
        params : numpy.ndarray
            Physical parameter values, length ``n_params``.  The ordering
            must match :attr:`param_names` exactly.

        Returns
        -------
        float
            Sum of log-prior values across all parameters.  Returns
            ``-inf`` immediately if any parameter falls outside its prior
            support (hard rejection).
        """
        total = 0.0
